...
- [ ] 降低首次使用门槛：服务端托管项目方 API Key 并转发模型请求，让普通求职者无需自行配置服务商、模型和密钥；部署方案、免费额度及调用成本待评估。
- [ ] 落实本地隐私存储：用户简历、收藏 JD 及后续工作流结果统一保存在本地，服务端不持久化业务内容；模型调用仍涉及必要输入的临时转发。
- [ ] 最小化使用统计：仅记录匿名安装实例口径的使用用户量，以及各模块使用、成功、失败次数，不采集业务内容。

以上均为后续规划，尚未完成实现与验收，范围和验收口径详见 [项目待办](docs/todo.md)。
